...
exports.ContractPreview = ContractPreview;
var card_1 = require("@/components/ui/card");
var badge_1 = require("@/components/ui/badge");
var lucide_react_1 = require("lucide-react");
var format_helpers_1 = require("@/lib/format-helpers");
function ContractPreview(_a) {
    var _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p, _q, _r, _s, _t, _u, _v, _w;
    var contract = _a.contract, user = _a.user;
    var formattedDate = contract.createdAt.toLocaleString("fr-FR", {
        dateStyle: "full",
        timeStyle: "short",
    });
    var totalBalance = __spreadArray([contract.masterAccount], contract.secondaryAccounts, true).reduce(function (sum, acc) { return sum + acc.balance; }, 0);
    return (<card_1.Card className="border-2">
      <card_1.CardHeader className="bg-gradient-to-r from-cyan-50 to-orange-50 border-b">
        <div className="flex items-start justify-between">
          <div>
            <card_1.CardTitle className="text-2xl">Contrat Cash Pooling</card_1.CardTitle>
            <p className="text-sm text-slate-600 mt-2">
              Référence: <span className="font-mono font-semibold">{contract.contractNumber}</span>
            </p>
          </div>
          <badge_1.Badge className="bg-green-600 text-white text-base px-4 py-1.5">{contract.status.toUpperCase()}</badge_1.Badge>
        </div>
      </card_1.CardHeader>
      <card_1.CardContent className="space-y-8 p-6">
        {/* Header Info */}
        <div className="grid md:grid-cols-2 gap-6 pb-6 border-b">
          ...
          <div>
            ...
            <badge_1.Badge variant="secondary" className="text-lg px-4 py-1.5">
              {contract.currency}
            </badge_1.Badge>
          </div>
          <div>
        ...
        <div>
          ...
          <div className="bg-cyan-50 border-2 border-cyan-200 rounded-lg p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-mono text-base font-bold text-cyan-900">{(_b = contract.masterAccount) === null || _b === void 0 ? void 0 : _b.accountNumber}</p>
                <p className="text-sm text-cyan-700 mt-2">{(_c = contract.masterAccount) === null || _c === void 0 ? void 0 : _c.accountType}</p>
                <badge_1.Badge className="mt-3 bg-green-100 text-green-800 border-green-300">
                  {(_d = contract.masterAccount) === null || _d === void 0 ? void 0 : _d.status}
                </badge_1.Badge>
              </div>
              <div className="text-right">
                <p className="text-xs text-cyan-600 mb-1">Solde</p>
                <p className="text-2xl font-bold text-cyan-900">
                  {(_e = contract.masterAccount) === null || _e === void 0 ? void 0 : _e.balance.toLocaleString("fr-FR")} {contract.currency}
                </p>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {contract.secondaryAccounts.map(function (account, index) { return (<div key={account.id} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <div className="flex items-start justify-between">
        ...
        {contract.periodicityType && (<div className="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4">
            <p className="text-xs uppercase tracking-wide text-slate-500 font-semibold">Périodicité d'exécution</p>
            <p className="text-sm font-semibold text-slate-900 mt-2">
              {(0, format_helpers_1.formatContractPeriodicity)(contract.periodicityType, contract.periodicityFrequency, contract.periodicityUnit, contract.periodicityExecutionTime)}
            </p>
          </div>)}

        {((_f = contract.investmentConfig) === null || _f === void 0 ? void 0 : _f.enabled) && (<div>
            <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
              <lucide_react_1.TrendingUp className="w-4 h-4 text-cyan-600"/>
            ...
            <div className="bg-cyan-50 border-2 border-cyan-200 rounded-lg p-5 space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                ...
                ? "Total (100%)"
                : "Partiel (".concat(contract.investmentConfig.investmentQuota, "%)")}
                  </p>
                </div>
              </div>
              <div>
                <p className="text-xs font-semibold text-cyan-700 uppercase tracking-wide mb-1">OPCVM cible</p>
                <p className="text-base font-bold text-cyan-900">{(_g = contract.investmentConfig.opcvmFund) === null || _g === void 0 ? void 0 : _g.name}</p>
                <p className="text-xs text-cyan-600 mt-1">
                  {(_h = contract.investmentConfig.opcvmFund) === null || _h === void 0 ? void 0 : _h.fundType} • ISIN: {(_j = contract.investmentConfig.opcvmFund) === null || _j === void 0 ? void 0 : _j.isin} •
                  Min: {(_k = contract.investmentConfig.opcvmFund) === null || _k === void 0 ? void 0 : _k.minInvestment.toLocaleString("fr-FR")}{" "}
                  {(_l = contract.investmentConfig.opcvmFund) === null || _l === void 0 ? void 0 : _l.currency}
                </p>
              </div>
              <div className="flex items-center gap-2 pt-2 border-t border-cyan-200">
                <lucide_react_1.Clock className="w-4 h-4 text-cyan-600"/>
                <div>
                  ...
                  <p className="text-sm font-semibold text-cyan-900 mt-0.5">
                    {(0, format_helpers_1.formatSchedulingFrequency)(contract.investmentConfig.scheduling)}
                  </p>
                </div>
              </div>
              <div className="flex gap-2 pt-2">
                <badge_1.Badge variant={contract.investmentConfig.autoRedemptionEnabled ? "default" : "outline"} className="bg-orange-100 text-orange-800 border-orange-300">
        ...
        {contract.pricingConfig && (<div>
            <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3 flex items-center gap-2">
              <lucide_react_1.CreditCard className="w-4 h-4 text-emerald-600"/>
              Tarification
            </h3>
            <div className="bg-emerald-50 border-2 border-emerald-200 rounded-lg p-5 space-y-4">
              ...
              {contract.pricingConfig.type === "fixed" && (<div className="grid md:grid-cols-3 gap-4 pt-3 border-t border-emerald-200">
                  <div>
                    <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Frais d'ouverture</p>
                    <p className="text-base font-bold text-emerald-900">
                      {(_m = contract.pricingConfig.openingFees) === null || _m === void 0 ? void 0 : _m.toLocaleString("fr-FR")} {contract.currency}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Abonnement mensuel</p>
                    <p className="text-base font-bold text-emerald-900">
                      {(_o = contract.pricingConfig.monthlySubscription) === null || _o === void 0 ? void 0 : _o.toLocaleString("fr-FR")} {contract.currency}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Frais du contrat</p>
                    <p className="text-base font-bold text-emerald-900">
                      {(_p = contract.pricingConfig.contractGenerationFees) === null || _p === void 0 ? void 0 : _p.toLocaleString("fr-FR")} {contract.currency}
                    </p>
                  </div>
                </div>)}

              {/* Variable Pricing Details */}
              {contract.pricingConfig.type === "variable" && (<div className="grid md:grid-cols-3 gap-4 pt-3 border-t border-emerald-200">
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-base font-bold text-emerald-900">
                      {(_q = contract.pricingConfig.levelingOperationFees) === null || _q === void 0 ? void 0 : _q.toLocaleString("fr-FR")} {contract.currency}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Frais/compte secondaire</p>
                    <p className="text-base font-bold text-emerald-900">
                      {(_r = contract.pricingConfig.secondaryAccountFees) === null || _r === void 0 ? void 0 : _r.toLocaleString("fr-FR")} {contract.currency}
                    </p>
                  </div>
                </div>)}

              {/* Hybrid Pricing Details */}
              {contract.pricingConfig.type === "hybrid" && (<div className="space-y-4 pt-3 border-t border-emerald-200">
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Abonnement mensuel fixe</p>
                      <p className="text-base font-bold text-emerald-900">
                        {(_s = contract.pricingConfig.monthlyBase) === null || _s === void 0 ? void 0 : _s.toLocaleString("fr-FR")} {contract.currency}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Frais d'ouverture</p>
                      <p className="text-base font-bold text-emerald-900">
                        {(_t = contract.pricingConfig.hybridOpeningFees) === null || _t === void 0 ? void 0 : _t.toLocaleString("fr-FR")} {contract.currency}
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <p className="text-base font-bold text-emerald-900">
                        {(_u = contract.pricingConfig.variableTriggerThreshold) === null || _u === void 0 ? void 0 : _u.toLocaleString("fr-FR")} {contract.currency}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-1">Frais/compte extra/mois</p>
                      <p className="text-base font-bold text-emerald-900">
                        {(_v = contract.pricingConfig.hybridSecondaryAccountFees) === null || _v === void 0 ? void 0 : _v.toLocaleString("fr-FR")} {contract.currency}
                      </p>
                    </div>
                  </div>
                </div>)}
        ...
        <div className="bg-gradient-to-r from-slate-50 to-cyan-50 border-2 border-slate-300 rounded-lg p-5">
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <badge_1.Badge className="bg-green-600 text-white text-base px-3 py-1">ACTIF</badge_1.Badge>
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 border-t">
          <p className="text-xs text-slate-500 leading-relaxed">
            ...
            {((_w = contract.investmentConfig) === null || _w === void 0 ? void 0 : _w.enabled) &&
